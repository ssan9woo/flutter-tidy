...
suite('Asset Analyzer Tests', () => {
  ...
  test('Should detect unused assets correctly', () => {
    const result = analyzeAssetUsage(tempDir);
    ...
    assert.strictEqual(result.usedAssets.length, 3);
  ...
  test('Should detect assets referenced in various string formats', () => {
    const result = analyzeAssetUsage(tempDir);
